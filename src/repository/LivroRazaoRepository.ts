...
export class LivroRazaoRepository {
    private static instance: LivroRazaoRepository;
    private constructor() { }

    public static getInstance(): LivroRazaoRepository {
    ...
    public async getSaldoAnterior(id_conta: number, dataInicio: string, id_empresa: number): Promise<number> {
        const sql = `
    ...
    `;
        const res = await executarComandoSQL(sql, [id_conta, id_empresa, dataInicio]);
        return res[0]?.saldo ? Number(res[0].saldo) : 0;
    }

    // Busca movimentações do período
    public async getMovimentacao(id_conta: number, dataInicio: string, dataFim: string, id_empresa: number): Promise<any[]> {
        const sql = `
    ...
    `;
        return await executarComandoSQL(sql, [id_conta, id_empresa, dataInicio, dataFim]);
    }
}